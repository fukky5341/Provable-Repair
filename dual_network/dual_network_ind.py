...
class DualNetwork_Ind:
        ...
        for layer_idx, layer in enumerate(self.ori_net):
            if isinstance(layer, nn.Linear):
                ...
                dual_layer = DualConv2D_Ind(
                    layer,
                    in_shape=self.shapes[layer_idx],
                    out_shape=self.shapes[layer_idx + 1],
                )
    ...
    # ------------------------------------------------------------
    # Estimation of iput splitting
    # ------------------------------------------------------------
    def estimate_input_split_score(self, A):
        lb = self.lbs[0].view(-1)
        ub = self.ubs[0].view(-1)
        width = ub - lb  # (n,)
        if A.dim() == 1:
            A = A.unsqueeze(0)  # (1, n)
        
        # option 1: sum of absolute values of A across output dimensions, multiplied by input width
        # score = torch.abs(A).sum(dim=0) * width * 0.5  # (n,) <-- unnecessary to multiply by 0.5
        score = torch.abs(A).sum(dim=0) * width  # (n,)

        # # option 2: take max value across output dimensions, multiplied by input width
        # score = torch.abs(A).max(dim=0).values * width * 0.5  # (n,)

        return score  # higher = better split
    
    def best_input_split(self, topk=5):
        if self.As is None or len(self.As) == 0:
            raise ValueError("As is not computed yet.")
        input_A = self.As[-1]
        score = self.estimate_input_split_score(input_A)
