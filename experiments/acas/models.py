...
def acas(
    ...
    # )
    filename = f"ACASXU_run2a_{a_prev}_{tau}_batch_2000.nnet"
    filepath = Path(__file__).resolve().parents[2] / "models" / "acasxu" / filename
    if not filepath.exists():
        raise FileNotFoundError(f"Model file not found: {filepath}")
    network, normalize, denormalize = sytorch.nn.from_file(filepath.as_posix())
